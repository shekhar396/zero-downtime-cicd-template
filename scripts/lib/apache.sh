...
apache_resolve_public_port() { apache_service_field "$1" public_port "${2:-$SERVICE_CONFIG_FILE}"; }
...
apache_render_template() {
  ...
  local active_color active_upstream_port public_port server_name
  [[ -f "$APACHE_TEMPLATE_FILE" ]] || { apache_error "template not found: $APACHE_TEMPLATE_FILE"; return 1; }
  public_port="$(apache_resolve_public_port "$service_name" "$config_file")"
...
apache_static_validate_file() {
  local file="$1" config_file="${2:-$SERVICE_CONFIG_FILE}" errors=0 service_name public_port
  ...
  grep -Eq '<VirtualHost[[:space:]]+[^>]+>' "$file" || { apache_error "missing VirtualHost: $file"; errors=$((errors + 1)); }
  service_name="$(awk -F': ' '$0 ~ /^# service: / { print $2; exit }' "$file")"
  if [[ -z "$service_name" ]]; then
    apache_error "missing service header: $file"
    errors=$((errors + 1))
  else
    if public_port="$(apache_resolve_public_port "$service_name" "$config_file" 2>/dev/null)"; then
      grep -Eq "<VirtualHost[[:space:]]+[^>]*:${public_port}>" "$file" || { apache_error "missing configured public_port $public_port in VirtualHost for $service_name: $file"; errors=$((errors + 1)); }
    else
      apache_error "cannot resolve configured public_port for $service_name from $config_file"
      errors=$((errors + 1))
    fi
  fi
...
apache_validate_generated_file() {
  local generated_config_file="$1" service_config_file="${2:-$SERVICE_CONFIG_FILE}"
  [[ -f "$generated_config_file" ]] || { apache_error "config file not found: $generated_config_file"; return 1; }
  [[ "$generated_config_file" == *.conf ]] || { apache_error "config file must end with .conf: $generated_config_file"; return 1; }
  apache_static_validate_file "$generated_config_file" "$service_config_file" || return 1
  apache_syntax_validate
}

apache_validate_generated_dir() {
  local config_dir="$1" service_config_file="${2:-$SERVICE_CONFIG_FILE}" file found=0 errors=0
  [[ -d "$config_dir" ]] || { apache_error "config directory not found: $config_dir"; return 1; }
  config_dir="$(cd "$config_dir" && pwd)"
  while IFS= read -r file; do
    apache_is_generated_file "$file" || continue
    found=1
    apache_static_validate_file "$file" "$service_config_file" || errors=$((errors + 1))
...
apache_validate_generated_path() {
  local config_path="$1" service_config_file="${2:-$SERVICE_CONFIG_FILE}"
  if [[ -f "$config_path" ]]; then
    apache_validate_generated_file "$config_path" "$service_config_file"
  elif [[ -d "$config_path" ]]; then
    apache_validate_generated_dir "$config_path" "$service_config_file"
  else
